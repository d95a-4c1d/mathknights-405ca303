...
import { CheckCircle2, ArrowRight, TrendingUp, Package, Target, RotateCcw } from 'lucide-react';
...
export default function Result() {
  ...
  const game = useGame();
  const { problem, stageName, rewards } = (location.state || {}) as {
    problem: Problem; stageName: string; rewards: RewardItem[];
  };
  ...
  return (
    <div className="relative w-screen min-h-screen bg-background flex flex-col items-center justify-center px-4 py-8">
      <div className="watermark text-[10vw] top-[10%] left-[5%]">COMPLETE</div>
      ...
      >
        <div className="text-center mb-6">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.15, type: 'spring' }}
            className="w-16 h-16 mx-auto mb-3 rounded-2xl icon-block-orange flex items-center justify-center"
            style={{ boxShadow: '0 8px 24px hsl(25 95% 53% / 0.25)' }}
          >
            <CheckCircle2 className="w-8 h-8" />
          </motion.div>
          <div className="text-xl font-semibold tracking-wide">挑战成功</div>
          <div className="text-muted-foreground text-xs font-mono mt-1">{stageName}</div>
        </div>

        <div className="card-inset p-3 rounded-xl mb-4">
          <div className="text-[10px] font-mono text-muted-foreground mb-1">已完成</div>
