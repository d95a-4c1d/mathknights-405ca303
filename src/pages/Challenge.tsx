...
export default function Challenge() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.correct) {
        ...
        game.refreshProfile();
        navigate('/result', { state: { problem, stageName, rewards, isCustom, feedback: result.feedback, competencyDelta: result.competencyDelta ?? null } });
      } else {
  ...
  return (
    <div className="relative w-screen min-h-screen md:h-screen md:overflow-hidden bg-background flex flex-col">
      <div className="absolute bottom-0 right-2 text-[22vw] md:text-[16vw] font-display font-black text-white/[0.04] select-none pointer-events-none leading-none z-0">CHALLENGE</div>
